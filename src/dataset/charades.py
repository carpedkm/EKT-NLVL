...
class CharadesDataset(AbstractDataset):
    ...
    def collate_fn(self, data):
        ...
        #                 random_range = random.uniform(0, 0.3 * float(b['duration']))
        #                 if st >= random_range:
        #                     vid_ft_start_new = random_range
                            
        #                 elif st < random_range:
        #                     vid_ft_start_new = st

                        
        #                 new_start= st 
        #                 new_end = end 
        #                 new_duration = float(b['duration']) - vid_ft_start_new
        #                 vid_ft_end_new = float(b['duration'])
                        
        #             elif apply_vda_loc[0] == 'End':
        #                 random_range = random.uniform(0.7 * float(b['duration']), float(b['duration']))
        #                 if end <= random_range:
        #                     vid_ft_end_new =  random_range
        #                 elif end > random_range:
        #                     vid_ft_end_new = end
        #                 vid_ft_start_new = 0    
        #                 new_start = st
        #                 new_end = end
        ...
        #             # fix the batch
        #             duration_origin = float(b['duration'])
        #             b['duration'] = torch.tensor(new_duration)
        #             b['timestamps'] = torch.tensor([float(new_start - vid_ft_start_new), \
        #                                 float(new_end - vid_ft_start_new)])
                    
        #             b['grounding_start_pos'] = torch.tensor((new_start - vid_ft_start_new) / new_duration)
        #             b['grounding_end_pos'] = torch.tensor((new_end - vid_ft_start_new) / new_duration)
        #             b['grounding_att_masks'] = torch.zeros((128))
        #             # b_ones = torch.ones((128))
        #             nfeats_old = b['nfeats'] 
        #             b['nfeats'] = torch.tensor(int(np.round((vid_ft_end_new / duration_origin)* nfeats_old)) - int(np.round((vid_ft_start_new / duration_origin)* nfeats_old)))
        #             b['grounding_att_masks'][int(np.round((b['nfeats'] * b['grounding_start_pos']))):int(np.round(b['nfeats']* b['grounding_end_pos']))] = torch.tensor(1)
                    
        #             # tmp_vid_feats = b['video_feats'][:int(nfeats_old),:]
        #             tmp_vid_feats = b['video_feats'][int(np.round((vid_ft_start_new / duration_origin) * nfeats_old)):int(np.round((vid_ft_end_new / duration_origin) * nfeats_old)),:]
                    
        #             assert int(np.round((vid_ft_end_new / duration_origin)* nfeats_old)) - int(np.round((vid_ft_start_new / duration_origin)* nfeats_old)) == int(b['nfeats'])
